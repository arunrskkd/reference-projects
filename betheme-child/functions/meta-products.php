 ...
 * version 1.2 | Layouts
 *-----------------------------------------------------------------------------------*/
if( ! function_exists( 'mfn_products_meta_add' ) )
{
	function mfn_products_meta_add(){
		global $mfn_products_meta_box;
		...
		$mfn_products_meta_box = array(
			'id' 		=> 'mfn-meta-products',
		...
		add_meta_box($mfn_products_meta_box['id'], $mfn_products_meta_box['title'], 'mfn_products_show_box', $mfn_products_meta_box['page'], $mfn_products_meta_box['context'], $mfn_products_meta_box['priority']);
	}
}
add_action('admin_menu', 'mfn_products_meta_add');


/*-----------------------------------------------------------------------------------*/
/*	Callback function to show fields in meta box
/*-----------------------------------------------------------------------------------*/
if( ! function_exists( 'mfn_products_show_box' ) )
{
	function mfn_products_show_box() {
		global $MFN_Options, $mfn_products_meta_box, $post;
		$MFN_Options->_enqueue();
	 	
		// Use nonce for verification
		echo '<div id="mfn-wrapper">';
			echo '<input type="hidden" name="mfn_products_meta_nonce" value="', wp_create_nonce(basename(__FILE__)), '" />';
			...
			echo '<table class="form-table">';
				echo '<tbody>';		
					...
					foreach ($mfn_products_meta_box['fields'] as $field) {
...
/*-----------------------------------------------------------------------------------*/
if( ! function_exists( 'mfn_products_save_data' ) )
{
	function mfn_products_save_data($post_id) {
		global $mfn_products_meta_box;
	 
		// verify nonce
		if( key_exists( 'mfn_products_meta_nonce',$_POST ) ) {
			if ( ! wp_verify_nonce( $_POST['mfn_products_meta_nonce'], basename(__FILE__) ) ) {
				return $post_id;
			}
		}
		...
		// check and save fields ( $mfn_products_meta_box['fields'] )
		foreach ( (array)$mfn_products_meta_box['fields'] as $field ) {
...
add_action('save_post', 'mfn_products_save_data');
